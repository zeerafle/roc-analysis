...
<template>
  <div class="flex flex-col items-center gap-2">
    <div class="flex gap-4 bg-gray-50 p-2 rounded border text-xs w-full justify-center">
      <div class="flex flex-col gap-1 w-32">
        <label class="font-bold flex justify-between"><span>Positives (P)</span> <span>{{ numPos }}</span></label>
        <input type="range" v-model="numPos" min="50" max="500" step="50" class="h-1" />
      </div>
      <div class="flex flex-col gap-1 w-32">
        <label class="font-bold flex justify-between"><span>Negatives (N)</span> <span>{{ numNeg }}</span></label>
        <input type="range" v-model="numNeg" min="100" max="5000" step="100" class="h-1" />
      </div>
      <div class="flex flex-col gap-1 w-32">
        <label class="font-bold flex justify-between"><span>Separation (d')</span> <span>{{ separation }}</span></label>
        <input type="range" v-model="separation" min="0.5" max="3" step="0.1" class="h-1" />
      </div>
      <div class="flex flex-col justify-center border-l pl-4">
        <div class="font-bold">Skew Ratio (N:P)</div>
        <div class="text-lg text-center text-purple-600 font-mono">1 : {{ (numNeg/numPos).toFixed(1) }}</div>
      </div>
    </div>
